...
//if($total['total'] > 40	)
//t3lib_utility_Debug::debug($total, 'tx_cfcleaguefe_views_MatchMarker'); // TODO: Remove me!
...
//		return $formatter->cObj->substituteMarkerArrayCached($template, $markerArray, $subpartArray, $wrappedSubpartArray);
	...
	private function prepareFields(&$match) {
		...
		//    t3lib_utility_Debug::debug($match->record['lineup_home'], 'tx_cfcleaguefe_util_MatchMarker');
	}
...
//      t3lib_utility_Debug::debug($typeArr, '#'. $dynaMarkers[$i] . '# tx_cfcleaguefe_util_MatchMarker');
		}
	}
